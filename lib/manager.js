...
function Manager(argv, run_config={}){
    ...
    try {
        ...
        this.config_ts = Object.keys(explicit_mgr_opt).length ? date() :
            conf.ts;
...
function*(_this, old_proxy, new_proxy){
    ...
    _this.set_config_changes('proxies.'+old_proxy.port, new_proxy);
    yield _this.config.save();
...
function*(_this, old_proxy, new_proxy){
    ...
    _this.set_config_changes('proxies.'+new_proxy.port, new_proxy);
    const {proxy_port, proxy_err} = yield _this.create_new_proxy(proxy);
    if (proxy_err)
    {
...
function*refresh_zones(_this, req, res, next){
    this.on('uncaught', next);
    ...
    res.status(400).send('You need to sign in again');
});
...
function*mgr_creds(_this, req, res, next){
    ...
    yield _this.lpm_f.login({apply_cloud_config: 1});
...
E.prototype.proxies_running_get_api = function(req, res){
    const proxies = Object.values(this.proxy_ports).map(p=>{
        ...
        return config;
    }).filter(p=>p.port!=this._defaults.dropin_port)
...
E.prototype.authenticate = function(req, res, next){
        ...
        '/last_version', '/lpm_stats'].includes(req.url))
    {
        ...
        return void res.send('This LPM instance is running in local_login '
            +'mode. You need to sign in to get an access to this resource');
    }
    next();
};
...
E.prototype.create_api = function(){
    ...
    app.get('/proxies/:port', (req, res)=>{
        const port = +req.params.port;
        for (let index = 0; index < this.proxies.length; index++) {
            const p = this.proxies[index];
            if (p.port == port) {
                return res.json(p)
            }
        }
...
E.prototype.run_workers = function(){
    const cores = os.cpus().length;
    const num_workers = this.argv.cluster===true ?
        cores : Number(this.argv.cluster)||1;
...
E.prototype.resolve_proxies = etask._fn(function*(_this){
    ...
    if (_this.conn.current_country=='cn' || _this.argv.cn)
        _this.cn_hosts = yield _this.api_resolve_proxies({cn: 1});
});

E.prototype.api_resolve_proxies = etask._fn(function*(_this, opt={}){
    try {
        const response = yield _this.api_request({
            endpoint: '/lpm/resolve_super_proxy',
            qs: {limit: 20, cn: !!opt.cn},
            force: true,
        });
...
E.prototype.set_logger_level = function(level, from_argv){
    ...
    Object.values(cluster.workers).forEach(w=>{
        try {
            w.send({code: 'SETUP', level, customer: this._defaults.customer});
        } catch(e){
